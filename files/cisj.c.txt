...
Data da última atualização: 16/01/2023 - versão: 8
Funcionalidade: Detector de falhas vCube no ambiente de simulação SMPL
*/
...
//Estrutura dos eventos para apresentar latência e demais dados a cada eventp
typedef struct {
    int tipo;
    int latencia;
    int rodada;
...
int atualizaEvento(st_event  *events, int node) {
    ...
    if (events[node].tipo == FAULT) {
        if (events[node].node_event == 1 ) {
            for (j = 0; j < n; j++) {
                if (info_no[j] == 1 && ((no[j].states[node] % 2) != 1) && i !=j)
                return 0;
            }
        }
        return 1;
    ...
    //Se é RECOVERY
        ...
        if (events[node].node_event == 1 ) {
            for (j = 0; j < n; j++) {
                if (info_no[j] == 1 && ((no[j].states[node] % 2) != 0) && i !=j)
                return 0;
            }
        }
        return 1;
...
int main (int argc, char *argv[]) {
    ...
    if (argc != 3) {
        puts ("Uso correto: ./cisj <num_processos> <tempo_simulacao>\n");
        puts ("Exemplo: ./cisj 8 300 \n");
        exit(1);
    }
    ...
    schedule(FAULT, 130.0, 1);
    schedule(FAULT, 180.0, 3);
    schedule(FAULT, 200.0, 5);
    schedule(FAULT, 250.0, 7);
    schedule(RECOVERY, 250.0, 1);
    schedule(RECOVERY, 300.0, 3);
    schedule(RECOVERY, 350.0, 5);
    ...
    puts("======================================================================================"); 
    puts("Início da execução: Trabalho 1 - vCube.");
    puts("Implementação de um detector de falhas vCube no ambiente de simulação SMPL.");
